...
<table style="background-color: #6468e7; border: 0;">
	<tr>
		<td bgcolor="#FFFF88"><a href="<c:url value="/viewCategory.do">
			<c:param name="categoryName" value="${categoryName}"/></c:url>">
...
<p align="center">
	...
	<table style="background-color: #6468e7; border: 0;vertical-align: middle;">
			...
			<tr bgcolor="#FFFF88">
				<td><b><a href="<c:url value="/viewItem.do"><c:param name="itemName" value="${item.itemName}"/></c:url>"> 
							<c:out value="${item.itemName}" />
					 </a></b></td>
				<td><c:out value="${product.productNumber}" /></td>
				<td><c:out value="${item.attr1}" /> <c:out value="${item.attr2}" /> <c:out value="${item.attr3}" /> 
					<c:out value="${item.attr4}" /> <c:out value="${item.attr5}" /> <c:out value="${product.productName}" />
				</td>
				<td> <fmt:formatNumber value="${item.listPrice}" pattern="$#,##0.00" /> </td>
				<td valign="bottom"> <html:link paramId="workingItemName" paramName="item" paramProperty="itemName"
						page="/addItemToCart.do" transaction="true">
